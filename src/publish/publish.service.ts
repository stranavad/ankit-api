...
import { QuestionToPublish, selectPublishedQuestionnaire, selectQuestionsToPublish } from './publish.interface';
...
export class PublishService {
    ...
    async updatePublishedQuestionnaire(publishedId: number, body: UpdatePublishedQuestionnaireDto){
        const updateData = {name: body.name}
        const updatedQuestionnaire = await this.prisma.publishedQuestionnaire.update({
            where: {
                id: publishedId
            },
            data: updateData,
    ...
    async checkQuestionnairePublish(questionnaireId: number): Promise<{lastPublished: Date | null, canPublish: boolean}>{
    ...
    async loadPublishedQuestionnaires(questionnaireId: number){
        const publishedQuestionnaires = await this.prisma.publishedQuestionnaire.findMany({
            where: {
                questionnaireId
            },
            select: selectPublishedQuestionnaire,
    ...
    async publishQuestionnaire(questionnaireId: number, memberId: number, data: UpdatePublishedQuestionnaireDto){
        ...
        if(lastPublish){
            const matchedQuestionsMap = new Map<number, {id: number, publishedAt: Date, questionId: number}>()
            lastPublish?.questions.map((question) => matchedQuestionsMap.set(question.questionId, question));
    
    
            lastPublish?.questionnaire.questions.map((question) => {
                const mappedQuestion = matchedQuestionsMap.get(question.id);
    
                if(!mappedQuestion || dayjs(question.updated).isAfter(mappedQuestion.publishedAt)){
        ...
        const publishedQuestionnaireData = Prisma.validator<Prisma.PublishedQuestionnaireCreateArgs>()({
            data: {
                name: data.name,
                questionnaire: {
                    connect: {
        ...
        const newPublish = await this.prisma.publishedQuestionnaire.create(publishedQuestionnaireData)
